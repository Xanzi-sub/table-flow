...
export function CustomerAppBridge({ customerSessionId }: { customerSessionId: string }) {
  ...
  return <>
    {ringing && <div className="fixed inset-x-3 bottom-5 z-[120] mx-auto flex max-w-md items-center justify-between gap-3 rounded-2xl border border-red-200 bg-white p-4 shadow-2xl"><div><p className="text-sm font-bold">Your order was updated</p><p className="text-xs text-neutral-500">Open your order to see the latest status.</p></div><button onClick={silence} className="rounded-full border border-red-300 px-4 py-2 text-xs font-bold text-red-700">Got it</button></div>}
